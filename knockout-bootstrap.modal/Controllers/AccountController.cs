namespace knockoutbootstrapmodal.Controllers
{
	using System;
	...
	using knockoutbootstrapmodal.Filters;
	...
	public class AccountController : Controller
	{
		...
		{
			var result =
				OAuthWebSecurity.VerifyAuthentication(this.Url.Action("ExternalLoginCallback", new { ReturnUrl = returnUrl }));
				...
				if (this.ModelState.IsValid)
				{
					...
					catch (Exception)
					{
						this.ModelState.AddModelError(
							string.Empty, 
							string.Format(
								"Unable to create local account. An account with the name \"{0}\" may already exist.", this.User.Identity.Name));
					}
				}
			}
